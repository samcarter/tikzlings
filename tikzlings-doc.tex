...
\date{Version v2.6 \textendash{} 2026/08/20}
...
\newcommand{\tikzlinghookforeground}{\path (-1.63,0.1) rectangle (1.63,0.1);}

\def\battext{bat}
\newcommand{\tocentrysam}[3]{%
  % 1: label
  % 2: title
  % 3: tikz code
  \renewcommand{\tikzlinghookforeground}{}%
  \hyperref[#1]{%
    ~~\begin{tikzpicture}[scale=1]
      \path (0,0) rectangle (0,2.6);
      #3
      \node at (0,-0.25) {\strut\MakeTitlecase{#2}};
      \node at (0,-0.8) {p.\,\pageref{#1}};
    \end{tikzpicture}~~%
  }\hfil%
}
...
\begingroup
  ...
  \foreach \x in \tikzlings@names@clist {%
    \ifx\x\battext
      \tocentrysam{sec:\x}{\x}{\csname\x\endcsname[wings=0.5]}%
    \else
      \tocentrysam{sec:\x}{\x}{\csname\x\endcsname}%
    \fi
  }%
  \tocentrysam{sec:tikzlings}{Ti\emph{k}Zling}{
    \begin{scope}[scale=0.7,yshift=1.6cm,xshift=-0.3cm]
      \DeTriDim*[Theta=60,Phi=120]{3/6}
      \DeTriDim*[Origine={(1,-0.75)},Theta=60,Phi=100]{5/6}
    \end{scope}
  }%
  \tocentrysam{sec:things}{Accessories}{
    \thing[signpost={\TeX},scale=0.8]
    \thing[sombrero,rotate=20,yshift=-0.5cm]
    \thing[present]
  }%
  \par
\endgroup
...
With regard to the individual components of the \tikzlings, they are located in the following z order:\enlargethispage{0.5cm}
